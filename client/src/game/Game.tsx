...
export function JoinGame(props: { room: string }) {
  ...
  return (
    <ContextProvider room={props.room}>
      <Nav />
      <Show when={!ready()} fallback={<Game username={username()} />}>
...
function Game(props: { username: string }) {
  ...
  const socket = new WebSocket(
    `${(import.meta.env.PUBLIC_SERVER as string).replace(
      "http",
      "ws",
    )}/rooms/${connection.room}?${params.toString()}`,
  );
      ...
      .with({ type: "ConnectionUpdate" }, (message) => {
        const messageContent =
          message.state.type === "Connected" || message.state.type === "Reconnected"
            ...
            : `${
                connection.clients.find((client) => client.uuid === message.uuid)!.username
              } has left`;
