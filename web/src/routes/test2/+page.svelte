<script lang="ts">
	...
	$effect(() => {
		...
		activeOutlineElement
			...
			.finished.then(() => {
				const fromX2 = lerp(outlineX, playerX, 0.9);
				const fromY2 = lerp(outlineY, playerY, 0.9);
	...
	<div class="flex flex-1 items-center justify-center gap-4 overflow-y-hidden p-4 pt-0">
		...
		<div bind:this={activeOutlineElement} class="absolute left-0 top-0">
			<div
				...
				class="rotating-border size-48 transition-transform duration-[400ms]"
			></div>
		</div>
